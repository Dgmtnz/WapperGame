...
const HIT_STAGGER = 32.0
var LIFE = 15
var MAX_LIFE = 15  # Añade una vida máxima para el jugador
var HEAL_RATE = 1  # La cantidad de vida que el jugador recupera por segundo
var HEAL_DELAY = 4.0  # El tiempo en segundos antes de que el jugador comience a curarse
var time_since_hit = 0.0  # El tiempo transcurrido desde la última vez que el jugador fue golpeado
@onready var vida_label = $"CanvasLayer/Life_Label"
var score = 0  # Añade una variable para llevar la cuenta de la puntuación
...
func hit(dir):
	...
	LIFE -= 1
	vida_label.text = " \n\n VIDA = " + var_to_str(LIFE) + " Puntuacion: " + str(score)
	time_since_hit = 0.0  # Reinicia el temporizador cuando el jugador es golpeado
	if LIFE <= 0:
		get_tree().quit()
...
func _on_enemy_defeated():
	score = score + 1  # Incrementa la puntuación cuando un enemigo es derrotado
	vida_label.text = "\n\n VIDA = " + var_to_str(LIFE) + " Puntuacion: " + str(score)

func _on_heal_timer_timeout():
	# Esta función se llamará cada vez que el temporizador llegue a cero
	if LIFE < MAX_LIFE and time_since_hit >= HEAL_DELAY:
		LIFE += 1  # Incrementa la vida en 1
	vida_label.text = "\n\n VIDA = " + var_to_str(LIFE) + " Puntuacion: " + str(score)
